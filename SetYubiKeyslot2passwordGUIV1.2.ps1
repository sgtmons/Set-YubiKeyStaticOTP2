...
.VERSION       1.2.0
.AUTHOR        Michael Mercer mmercer@ocvibe.com https://github.com/sgtmons/Set-YubiKeyStaticOTP
...
- v1.1.3: Info Button handler bug & General Improvements 
    ...
    • Add Global Error trap for any uncaught fatal errors and added no YubiKey detected Windows Popup for Info button click handler.
- v1.1.4: General Improvements
    • Added Smart Card service status check—warn in log box if it’s disabled.
- v1.2: UI Improvements
    • Add Clear Clipboard Button
	• Add Past Password Button
- Known Issues: 
    • The random char string -:6usM+mU2` o[_ will not save. It adds a -1 to the end.
    • Static OTP values that end with an underscore character (_) may result in an **unexpected trailing "-l" or similar keystroke** being output by the YubiKey.
